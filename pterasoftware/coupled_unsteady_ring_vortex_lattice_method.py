...
CoupledUnsteadyRingVortexLatticeMethodSolver: A class used to solve
CoupledUnsteadyProblems with the unsteady ring vortex lattice method.
...
# TEST: Add integration tests for this class.
class CoupledUnsteadyRingVortexLatticeMethodSolver:
    """A class used to solve CoupledUnsteadyProblems with the unsteady ring vortex
    lattice method.
    ...
    **Contains the following methods:**
    ...
    calculate_solution_velocity: Finds the fluid velocity (in first Airplane's geometry
    axes, observed from the Earth frame) at one or more points (in first Airplane's
    geometry axes, relative to the first Airplane's CG) due to the freestream velocity
    ...
    def __init__(
        ...
        first_coupled_steady_problem = self.coupled_steady_problems[0]

        self.current_airplane = first_coupled_steady_problem.airplane
        self.current_coupled_operating_point = (
            first_coupled_steady_problem.coupled_operating_point
        )

        self.num_panels = self.current_airplane.num_panels
        ...
        self.stackR_pas_E_to_BP1 = np.zeros((self.num_steps, 3, 3), dtype=float)

        # Initialize attributes to hold state from MuJoCo for creating the next time
        # step's CoupledSteadyProblem.
        self._nextPosition_E_E: np.ndarray = np.empty(0, dtype=float)
        self._nextR_pas_E_to_BP1: np.ndarray = np.empty(0, dtype=float)
    ...
    def run(
        ...
        _num_spanwise_panels = 0

        for _wing in self.coupled_steady_problems[0].airplane.wings:
            _this_wing_num_spanwise_panels = _wing.num_spanwise_panels
            assert _this_wing_num_spanwise_panels is not None

            _num_spanwise_panels += _this_wing_num_spanwise_panels
        ...
        # Initialize list that will hold the approximate, relative times.
        approx_times = np.zeros(self.num_steps, dtype=float)
        for step in range(self.num_steps):
            # Calculate the total number of RingVortices analyzed during this step.
            num_wing_ring_vortices = _num_wing_panels
            num_wake_ring_vortices = self.list_num_wake_vortices[step]
            num_ring_vortices = num_wing_ring_vortices + num_wake_ring_vortices

            # REFACTOR: Once the coupled solver is stable, tune these multipliers
            #  empirically make the progress bar smooth and accurate.
        ...
        with tqdm(
            ...
            for step in range(self.num_steps):
                # Save attributes to hold the current step, Airplane, OperatingPoint,
                # and freestream velocity (in first Airplane's geometry axes,
                # observed from the Earth frame).
                self._current_step = step
                current_coupled_steady_problem = self.coupled_steady_problems[
                    self._current_step
                ]
                self.current_airplane = current_coupled_steady_problem.airplane
                self.current_coupled_operating_point = (
                    current_coupled_steady_problem.coupled_operating_point
                )
                ...
                logging.debug(
                    f"    speed = {self.current_coupled_operating_point.vCg__E:.2e} m/s"
                )
                logging.debug(
                ...
                _vCg_E__E = self.current_coupled_operating_point.vCg_E__E
                logging.debug(
                    f"    vCg_E__E = [{_vCg_E__E[0]:.2e}, {_vCg_E__E[1]:.2e}, "
                    f"{_vCg_E__E[2]:.2e}] m/s"
                )
                ...
                # steps, RingVortices are initialized after creating the next problem.
                if self._current_step == 0:
                    logging.info(
                        "    Initializing this time step's bound RingVortices."
                    )
                ...
                # the Airplane's geometry at this time step.
                ...
                # first Airplane's CG) on each Panel and the net loads on each Airplane.
                logging.info("    Calculating forces and moments.")
                self._calculate_loads()

                # Convert the loads to use MuJoCo's proper axes and reference points,
                # and then pass them to the MuJoCo model.
                logging.info("    Passing loads to MuJoCo.")
                self._pass_loads_to_mujoco()

                # Step the MuJoCo simulation forward by one time step.
                logging.info("    Stepping the MuJoCo simulation.")
                self.mujoco_model.step()

                logging.info("    Processing the new states from MuJoCo.")
                self._process_new_states_from_mujoco()

                logging.info("    Create the next time step's CoupledSteadyProblem.")
                self._create_next_coupled_steady_problem()

                # Initialize the next time step's bound RingVortices (if a next step
                # was created). This must be done before calling _populate_next_wake,
                # which accesses the next step's Panels' RingVortices.
                if self._current_step < self.num_steps - 1:
                    logging.info(
                        "    Initializing the next time step's bound RingVortices."
                    )
                    self._initialize_panel_vortices(self._current_step + 1)

                # Shed RingVortices into the wake.
                logging.info(
                    "    Shedding RingVortices into the wake of the next time step's "
    ...
    def _initialize_panel_vortices(self, step: int) -> None:
        ...
        time step, and then initializes them.
        ...
        for wing_id, wing in enumerate(this_airplane.wings):
            ...
            for chordwise_position in range(wing.num_chordwise_panels):
                for spanwise_position in range(_num_spanwise_panels):
                    ...
                    panel: _panel.Panel = _panels[chordwise_position, spanwise_position]
                    ...
                    # points based on whether the Panel is along the trailing edge or
                    # not.
                    if not panel.is_trailing_edge:
                        ...
                        if step == 0:
                            ...
                            Blrvp_GP1_CgP1 = (
                                _Blpp_GP1_CgP1
                                + thisVInf_GP1__E * self.delta_time * 0.25
                            )
                            Brrvp_GP1_CgP1 = (
                                _Brpp_GP1_CgP1
                                + thisVInf_GP1__E * self.delta_time * 0.25
                            ...
                            last_coupled_steady_problem = self.coupled_steady_problems[
                                step - 1
                            ]
                            last_airplane = last_coupled_steady_problem.airplane
    ...
    def _collapse_geometry(self) -> None:
        """Converts attributes of the CoupledUnsteadyProblem's geometry into 1D
        ndarrays.
        ...
        if self._current_step > 0:
            ...
            last_coupled_steady_problem: problems.CoupledSteadyProblem = (
                self.coupled_steady_problems[self._current_step - 1]
            )
            last_airplane = last_coupled_steady_problem.airplane

            # Iterate through the last time step's Airplane's Wings.
    ...
    def calculate_solution_velocity(
        ...
        relative to the first Airplane's CG) due to the freestream velocity and the
        induced velocity from every RingVortex.
    ...
    def _calculate_loads(self) -> None:
        ...
        **Notes:**
        ...
        for wing in self.current_airplane.wings:
            ...
            for panel in panels:
                ...
                if panel.is_right_edge:
                    ...
                    # difference between this Panel's RingVortex's strength, and the
                    # RingVortex's strength of the Panel to the right.
                    effective_right_line_vortex_strengths[global_panel_position] = (
                        self._current_bound_vortex_strengths[global_panel_position]
                ...
                if panel.is_leading_edge:
                    ...
                    # difference between this Panel's RingVortex's strength, and the
                    # RingVortex's strength of the Panel in front of it.
                    effective_front_line_vortex_strengths[global_panel_position] = (
                        self._current_bound_vortex_strengths[global_panel_position]
                ...
                if panel.is_left_edge:
                    ...
                    # difference between this Panel's RingVortex's strength, and the
                    # RingVortex's strength of the Panel to the left.
                    effective_left_line_vortex_strengths[global_panel_position] = (
                        self._current_bound_vortex_strengths[global_panel_position]
                ...
                if panel.is_trailing_edge:
                    if self._current_step == 0:
                        # Set the effective back LineVortex strength to this Panel's
                        # RingVortex's strength, as, for the first time step,
                        # there isn't a wake RingVortex to cancel it out.
                    ...
                    # difference between this Panel's RingVortex's strength, and the
                    # RingVortex's strength of the Panel to the back.
                    effective_back_line_vortex_strengths[global_panel_position] = (
                        self._current_bound_vortex_strengths[global_panel_position]
        ...
        rightLegForces_GP1 = (
            self.current_coupled_operating_point.rho
    ...
    def _pass_loads_to_mujoco(self) -> None:
        """Passes external and aerodynamic loads to MuJoCo for rigid body dynamics
        integration.

        **Notes:**

        The loads are specified in wind axes, then transformed to Earth axes for
        MuJoCo's world coordinate system.

        The transformation chain is: W_CgP1 > GP1_CgP1 > BP1_CgP1 > E_CgP1.

        :return: None
        """
        # Get the forces (in wind axes) on the Airplane.
        forces_W = self.current_airplane.forces_W
        assert forces_W is not None

        # Get the moments (in wind axes, relative to the first Airplane's CG) on the
        # Airplane.
    ...
    def _process_new_states_from_mujoco(self) -> None:
        ...
        **Notes:**
        ...
        Retrieves the updated position, orientation, velocity, and angular velocity from
        MuJoCo after the dynamics integration step. It transforms the velocity from
        Earth axes to the first Airplane's geometry axes, computes the new angle of
        attack and sideslip angle, and creates a new CoupledOperatingPoint for the next
        time step.

        The transformation chain for velocity is: E > BP1 > GP1.

        Stores the state for use by _create_next_coupled_steady_problem().

        :return: None
        """
        # Get the updated state from MuJoCo.
        state = self.mujoco_model.get_state()
        ...
        logging.debug(
            f"    next vCg_E__E = [{self._nextVelocity_E__E[0]:.2e}, "
            f"{self._nextVelocity_E__E[1]:.2e}, {self._nextVelocity_E__E[2]:.2e}] m/s"
        )

        # Compute the speed (magnitude of velocity in Earth frame).
        vCg__E = float(np.linalg.norm(self._nextVelocity_E__E))

        # Compute the freestream velocity (in Earth axes, observed from Earth frame).
        ...
        # Standard formulas:
        # - alpha (angle of attack): angle between body x-axis and projection of
        ...
        nu = self.current_coupled_operating_point.nu
        externalFX_W = self.current_coupled_operating_point.externalFX_W

        # Create a new CoupledOperatingPoint with the updated state.
        ...
        # Append the new CoupledOperatingPoint to the list.
        self.coupled_unsteady_problem.coupled_movement.coupled_operating_points.append(
            new_coupled_operating_point
        )

    def _create_next_coupled_steady_problem(self) -> None:
        """Creates the next time step's CoupledSteadyProblem with updated geometry.

        **Notes:**

        Makes a new Airplane with the updated position and orientation from MuJoCo, but
        preserves the Wing geometry from the prescribed motion (e.g., flapping). It then
        creates a new CoupledSteadyProblem with the updated Airplane and the
        CoupledOperatingPoint created in _process_new_states_from_mujoco().

        The position and orientation are obtained from the state stored by
        _process_new_states_from_mujoco().

        :return: None
        """
        # Check that we are not at the last time step.
        if self._current_step >= self.num_steps - 1:
            return

        # Get the pre-generated Airplane for the next time step from the
        # CoupledMovement. This Airplane has the correct Wing geometry based on
        # prescribed motion (e.g., flapping).
        next_step_id = self._current_step + 1
        prescribed_airplane = self.coupled_unsteady_problem.coupled_movement.airplanes[
            next_step_id
        ]

        # REFACTOR: Verify that reusing Wings from prescribed motion is correct. The
        #  relative geometry should be fine, but the wake RingVortices (stored in
        #  the Wings) have absolute coordinates in GP1_CgP1. Consider whether the
        ...
        # _process_new_states_from_mujoco.
        next_coupled_operating_point: operating_point.CoupledOperatingPoint = (
            self.coupled_unsteady_problem.coupled_movement.coupled_operating_points[
                next_step_id
            ]
        )

        # Create a new CoupledSteadyProblem with the updated Airplane and
        # CoupledOperatingPoint.
        next_coupled_steady_problem = problems.CoupledSteadyProblem(
            airplane=updated_airplane,
            coupled_operating_point=next_coupled_operating_point,
        )

        # Append the new CoupledSteadyProblem to the list.
        self.coupled_steady_problems.append(next_coupled_steady_problem)

    def _populate_next_airplanes_wake(self) -> None:
        """Updates the next time step's Airplane's wake.

        :return: None
        """
        # Populate the locations of the next time step's Airplane's wake RingVortex
        # points.
        self._populate_next_airplanes_wake_vortex_points()

        # Populate the locations of the next time step's Airplane's wake RingVortices.
        self._populate_next_airplanes_wake_vortices()

    def _populate_next_airplanes_wake_vortex_points(self) -> None:
        """Populates the locations of the next time step's Airplane's wake RingVortex
        points.
        ...
        **Notes:**
        ...
        if self._current_step < self.num_steps - 1:
            ...
            next_airplane = next_problem.airplane
            ...
            for wing_id, next_wing in enumerate(next_airplane.wings):
                ...
                if self._current_step == 0:
                    ...
                    for spanwise_panel_id in range(num_spanwise_panels):
                        ...
                        newRowWrvp_GP1_CgP1[0, spanwise_panel_id] = newFlwrvp_GP1_CgP1
    ...
    def _populate_next_airplanes_wake_vortices(self) -> None:
        ...
        **Notes:**
        ...
        if self._current_step < self.num_steps - 1:
            ...
            # Iterate through the next Airplane's predecessor's Wings.
            for wing_id, this_wing in enumerate(self.current_airplane.wings):
